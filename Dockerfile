...
RUN sed -i '248s/;enabled = false/enabled = true/' /etc/grafana/grafana.ini
RUN sed -i '249s/^;//' /etc/grafana/grafana.ini

ADD dashboards /var/lib/grafana/dashboards
ADD misc/Docker_System_Overview.json /var/lib/grafana/dashboards/Docker_System_Overview.json
